...
if __name__ == '__main__':
    ...
    for i in range(iters_num):
        ...
        if i % iter_per_epoch == 0:
            ...
            with open(f"{modelsAutoSavePath}//autosave_params_{epoch}.pkl", "wb") as f:
                pickle.dump(network.params, f)

            saved_models = sorted(os.listdir(modelsAutoSavePath),
                                  key=lambda x: os.path.getctime(os.path.join(modelsAutoSavePath, x)))
            autosave_models = [model for model in saved_models if
                               model.startswith("autosave_")]
            ...
            if base_loss is None:
                base_loss = loss
                with open(modelsAutoSavePath + "//best_model_params.pkl", "wb") as f:
                    pickle.dump(network.params, f)
            elif loss < base_loss:
                with open(modelsAutoSavePath + "//best_model_params.pkl", "wb") as f:
                    pickle.dump(network.params, f)
